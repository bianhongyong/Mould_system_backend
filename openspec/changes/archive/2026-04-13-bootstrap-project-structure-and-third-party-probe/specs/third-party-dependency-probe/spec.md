...
构建系统 MUST 定义统一依赖导入流程，覆盖 OSS SDK、MySQL 客户端、RabbitMQ 客户端、Boost、Abseil、gRPC、Protobuf。
...
工程 MUST 包含最小可执行目标（`third_party_probe`），并对后端服务依赖集合进行真实编译与链接验证。

#### Scenario: 探针目标可链接声明依赖

- **WHEN** 用户构建 `third_party_probe` 目标
- **THEN** 仅当当前环境中声明依赖的头文件与链接符号均有效时，构建 SHALL 成功
